...
import torchmetrics
import wandb
# Import the custom CBDNet
from CBD_Net_definition import CBDNet
...
# Data handling
from training.models.data_module import SARDataModule
...
class SARSegmentationModel(LightningModule):
    ...
    This model uses a CBD-Net model and class weights for handling the imbalanced dataset.
    """
    ...
    def __init__(self, learning_rate=5e-5, num_classes=5):
        ...
        # Define the custom CBD-Net model
        self.model = CBDNet(
            in_channels=1,    # Input channels (1 for SAR images)
            base_ch=64,       # The base number of channels
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.result_counter = save_prediction_images(masks, preds, counter=self.result_counter, result_folder='CBDNet_PR')
...
def main():
    ...
    # Initialize data module and your model
    data_module = SARDataModule(data_dir="dataset/", batch_size=batch_size, val_split=0.15)
    model = SARSegmentationModel(learning_rate=1e-4, num_classes=5)

    # Define checkpointing
    checkpoint_callback = ModelCheckpoint(
        monitor="val_iou_class_1", 
        mode="max", 
        save_top_k=1, 
    ...
    end_train = time.time()
    print(f'Total training time for {num_epochs} epochs, using batch size of {batch_size}: {end_train - start_time} seconds')

    # Save best model as a compressed zip file
